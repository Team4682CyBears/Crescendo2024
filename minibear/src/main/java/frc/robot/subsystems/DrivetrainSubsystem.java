...
public class DrivetrainSubsystem extends SubsystemBase {
  ...
  public static final double MIN_VELOCITY_BOUNDARY_METERS_PER_SECOND = MAX_VELOCITY_METERS_PER_SECOND * 0.14; // 0.14 a
                                                                                                              // magic
                                                                                                              // number
                                                                                                              // based
                                                                                                              // on
  ...
  private static final int PositionHistoryStorageSize = PositionHistoryWindowTimeMilliseconds
      / CommandSchedulerPeriodMilliseconds;
  ...
  public DrivetrainSubsystem() {
    ...
    // We assume the robot is level at startup. Take out any bias the NavX is
    // reading on Pitch/Roll.
    removePitchRollBias();
  }
   ...
   * NavX .getYaw() is -180 to 180 where positive is clockwise looking at the
   * sensor from above
   * According to
   * https://docs.wpilib.org/en/stable/docs/software/advanced-controls/geometry/coordinate-systems.html#field-coordinate-system
   * positive rotations are measured in the counter-clockwise looking at the robot
   * from above
   * these two pieces of information imply the yaw should be inverted (or
   * multiplied by -1.0)
   * 
   * @return A Rotation2d that describes the current orentation of the robot.
   */
  public Rotation2d getGyroscopeRotation() {
    // Note: for the navx1, isMagnetometerCalibrated was always false, and so the
    // code exercised the second path (using getYaw)
    // when we switched to the navx2, isMagnetometerCalibrated was true, and so the
    // code exercised the first path (using getFusedHeading)
    // this first path does not work for us because isMagtometerCalibrated does not
    // zero when yaw is zeroed
    // our ability to both zero and set the yaw is critical to field-oriented drive.
    // If we would like to us getFusedHeading in the future, we will need to change
    // how we zero and set the yaw.

    /*
     * if (swerveNavx.isMagnetometerCalibrated()) {
     * 
     * // System.out.
     * println("getGyroscopeRotation() using: swerveNavx.getFusedHeading()");
     * 
    ...
    // TODO test angleModulus again with the NavX1. We disabled it during debugging
    // the NavX2, but it might have been
    // a red herring.
    // return Rotation2d.fromRadians(
    // MathUtil.angleModulus(
   ...
   * 
   * @param historicDurationMilliseconds - the total milliseconds to look back in
   *                                     time
   * @return A double of the distance traveled by the robot in meters over the
   ...
   * @param historicDurationMilliseconds - the total milliseconds to look back in
   *                                     time
   * @return A double of the velocity of the robot in meters per second over the
   *         time window requested
   */
  ...
  /**
   * A method to obtain the average angular velocity of the robot recently
   * experienced
   * 
   * @param historicDurationMilliseconds - the total milliseconds to look back in
   *                                     time
   * @return A double of the angular velocity of the robot in radians per second
   *         over the time window requested
   */
   ...
   * @param historicDurationMilliseconds - the total milliseconds to look back in
   *                                     time
   * @return A double of the average acceleration experienced by the robot in
   *         meters/second^2 for the time window requested
   */
   ...
   * returns a new trajectory config so that when customization are made
   * downstream
  ...
  public void periodic() {
    ...
    if (swerveDriveMode == SwerveDriveMode.IMMOVABLE_STANCE && chassisSpeedsAreZero()) {
      ...
      // take the current 'requested' chassis speeds and ask the ask the swerve
      // modules to attempt this
      // first we build a theoretical set of individual module states that the
      // chassisSpeeds would corespond to
      ...
      // next we take the theoretical values and bring them down (if neecessary) to
      // incorporate physical constraints (like motor maximum speeds)
      SwerveDriveKinematics.desaturateWheelSpeeds(states, MAX_VELOCITY_METERS_PER_SECOND);
    }

    // now we take the four states and ask that the modules attempt to perform the
    // wheel speed and direction built above
    frontLeftModule.set(
  ...
  public void setRobotPosition(Pose2d updatedPosition) {
    // TOD this didn't work to fix the problem of the robot not getting correct
    // initial position.
  ...
  public void setSwerveDriveMode(SwerveDriveMode swerveDriveMode) {
    ...
    // if (swerveDriveMode == SwerveDriveMode.IMMOVABLE_STANCE) DO NOT set chassis
    // speeds to 0.0 here.
  ...
  public void zeroGyroscope() {
    if (swerveNavx.isCalibrating()) {
      // From the NavX Docs: This method has no effect if the sensor is currently
      // calibrating
  ...
  private ChassisSpeeds limitChassisSpeedsAccel(ChassisSpeeds speeds) {
    ...
    double omegaVelocityLimited = limitAxisSpeed(speeds.omegaRadiansPerSecond,
        previousChassisSpeeds.omegaRadiansPerSecond, MAX_ANGULAR_ACCELERATION_RADIANS_PER_SECOND_SQUARED);
   ...
   * 
   * @param currentRobotPosition - the centroid of the robot using the proper game
  ...
  /**
   * Helper method to obtain the SwerveModulePostion array from an existing
   * SwerveModules in this class
   * 
  ...
  /**
   * A function intended to be called from perodic to update the robots centroid
  ...
  private void refreshRobotPosition() {
    ...
    // getGyroscopeRotation() should be the same as currentPosition.getRotation
    // after one call to swerveOdometry.update in periodic
  ...
  /**
   * A method devoted to making sure the current position is added to the historic
   * position listing
   ...
   * 
   * @param historicDurationMilliseconds - the historic time window in
   *                                     milliseconds of points to establish
   *                                     translations
  ...
  private ArrayList<Transform2d> getRecentTransforms(int historicDurationMilliseconds) {
    ...
    for (Iterator<Pose2d> iter = this.historicPositions.iterator(); iter.hasNext()
        && currentCount < maxCount; ++currentCount) {
      currentPosition = iter.next();
      // assume the first position has no translation (even though it likely actually
      // have had one)
   ...
   * 
   * @param historicDurationMilliseconds - the historic time window in
   *                                     milliseconds to obtain velocities from
  ...
  private ArrayList<Double> getRecentAngularVelocities(int historicDurationMilliseconds) {
    ...
    for (int inx = 0; inx < transforms.size(); ++inx) {
      ...
      if (previousTransform != null) {
        ...
        // or said another way the robot can't physically spin > 25 spins per second -
        // 1500 RPM)
        // to produce a result, we need to rely on MathUtil.angleModulus() static method
        // - this keeps the math tidy
  ...
  private ArrayList<Double> getRecentVelocities(int historicDurationMilliseconds) {
    ...
    for (int inx = 0; inx < recentDistances.size(); ++inx) {
      ...
      double currentVelocity = currentDistance * CommandSchedulerCyclesPerSecond; // assumed cycle time and that the
                                                                                  // current distance is for a 20 ms
                                                                                  // movement
   ...
   * 
   * @param historicDurationMilliseconds - the historic time window in
   *                                     milliseconds to obtain accelerations from
  ...
  private ArrayList<Double> getRecentAccelerations(int historicDurationMilliseconds) {
    ...
    for (int inx = 0; inx < recentVelocities.size(); ++inx) {
      ...
      if (previousVelocity != null) {
        double currentAcceleration = (currentVelocity - previousVelocity) * CommandSchedulerCyclesPerSecond; // assumed
                                                                                                             // cycle
                                                                                                             // time and
                                                                                                             // that the
                                                                                                             // current
