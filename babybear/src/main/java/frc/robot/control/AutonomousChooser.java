...
public class AutonomousChooser {
    ...
    /**
     * Builds a command list for use in auto routines. This is the first part of the
     * routine that scores the game piece.
     * 
     * @param NodePosition
     * @param config       - when the auto will drive another trajectory after
     *                     scoring, supply a config with a higher ending velocity.
     * @return
     */
    private Command getScoreRoutine(Pose2d NodePosition, SwerveTrajectoryConfig config) {

        // Build into/out of node trajectories in real time because they depend on the
        // starting position
        ...
        command.addCommands(new InstantCommand(
                () -> System.out.println(
                        "Begin Driving Trajectory from: " + subsystems.getDriveTrainSubsystem().getRobotPosition())));

        // drive into node
        ParallelCommandGroup intoNodeAndHighScore = new ParallelCommandGroup();
        if (shouldDriveIntoNode) {
            intoNodeAndHighScore
                    .addCommands(new DriveTrajectoryCommand(subsystems.getDriveTrainSubsystem(), IntoNodeTrajectory));
        }
        ...
        // expell
        if (this.subsystems.getEveryBotPickerSubsystem() != null) {// cube uses uptake command to expell
                ...
                Map.ofEntries(
                        Map.entry(AutonomousBalance.DO_BALANCE,
                                new SequentialCommandGroup(
                                        new DriveTrajectoryCommand(subsystems.getDriveTrainSubsystem(),
                                                toRampTrajectory),
    ...
    private Command getDirectRoutine() {
        SequentialCommandGroup command = new SequentialCommandGroup();
        command.addCommands(
                getScoreAndDriveRoutine(trajectories.getNode5Position(), trajectories.getDirectToRampTrajectory()));
    ...
    private Command getMiddleRoutine() {
        SequentialCommandGroup command = new SequentialCommandGroup();
        command.addCommands(
                getScoreAndDriveRoutine(trajectories.getNode5Position(), trajectories.getMiddleTrajectoryPart1()));
        command.addCommands(new DriveTrajectoryCommand(subsystems.getDriveTrainSubsystem(),
                trajectories.getMiddleTrajectoryPart2()));
    ...
    private Command getNode2Routine() {
        SequentialCommandGroup command = new SequentialCommandGroup();
        command.addCommands(
                getScoreAndDriveRoutine(trajectories.getNode2Position(), trajectories.getNode2Trajectory()));
        command.addCommands(getBalanceRoutine(balanceChooser, trajectories.getLeftToOntoRampTrajectory()));
        return command;
    }
    ...
    private Command getNode8Routine() {
        SequentialCommandGroup command = new SequentialCommandGroup();
        command.addCommands(
                getScoreAndDriveRoutine(trajectories.getNode8Position(), trajectories.getNode8Trajectory()));
    ...
    private Command getRightRoutine() {
        SequentialCommandGroup command = new SequentialCommandGroup();
        command.addCommands(
                getScoreAndDriveRoutine(trajectories.getNode9Position(), trajectories.getRightTrajectory()));
        command.addCommands(getBalanceRoutine(balanceChooser, trajectories.getRightToOntoRampTrajectory()));
        return command;
    }
     ...
     * 
     * @param NodePosition starting position of robot corrosponding to the node.
     *                     Nodes are numbered from left to right 1- 9 from the
     *                     drivers perspective
    ...
    private void setRobotPose(SequentialCommandGroup command, Pose2d pose) {
        command.addCommands(
                // set yaw to the starting rotation here so that field orientation ends up
                // correct after auto
